...
DO, DL, DR = 0, 1, 2
def recurse(sx,sy):
    #if sy == y_max+2: #no where to go
    #    sand_set.add((sx,sy))
    #    return (sx, sy) # which means you need to stop
    # check if downnode is rock.
...
ix,iy = 500,0
init_sand_location = recurse(ix,iy)
#print(init_sand_location)

# go up one level from the sand location
rx, ry = init_sand_location

while not(rx == ix and ry == iy):
    sx, sy = rx, ry-1
